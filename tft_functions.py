...
def get_shop_names(img):
    ...
    for coord in roi_coords.champ_store_pos:
        ...
        # For each string obtained from OCR that is not a champ, match it to a champ name if it's similar enough. Add to master list
        for index, text in enumerate(shop_text):
            if text not in tft_assets.champions: 
                champs_in_shop.append(ocr.find_similar_name(text))
            else:
